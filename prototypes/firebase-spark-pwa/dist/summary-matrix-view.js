import{t as n,getLocale as f}from"./i18n/i18n.mjs";import{escapeHtml as e}from"./html-utils.js?v=20260808a";import{formatDietLabel as c}from"./diet-utils.mjs?v=20260810a";import{buildKitchenMatrixScreens as K,buildSummaryMatrixScreens as R}from"./summary-matrix-model.js?v=20260815f";function X(a,{days:t=[],operationDays:s=[],kitchen:r=!1,layout:m="classic",activeIndex:i=0,onActiveIndexChange:l=()=>{}}={}){const o=r?K(t,s):R(t,[],s);if(o.every(u=>u.columns.length===0)){a.innerHTML=`<p class="empty-state">${e(n("summary.noMeal"))}</p>`;return}const d=r?"kitchen":"summary",h=m==="international"?W:P;a.innerHTML=`
    ...
    <section class="summary-matrix-screen" data-${r}-screen="${a.index}" role="tabpanel" aria-hidden="${!m}">
      <table class="summary-matrix">
        ...
        <thead>
          ...
          <tr>
            ${a.columns.map(i=>`
              <th class="summary-matrix-meal-heading${p(i,a)}" scope="col"><span class="summary-matrix-meal-icon" aria-hidden="true">${S(i.mealTypeId)}</span><span class="summary-matrix-meal-label">${e(v(i))}</span>${H(i,t)}</th>
            `).join("")}
          </tr>
        </thead>
        <tbody>
          ${a.hasGuestGroup?y(n("summary.guests"),"summary-matrix-row-guests",a,i=>String(i.guestCount)):""}
          ${y(n("summary.diningMeals"),"summary-matrix-row-meals",a,D)}
  ...
  `}function E(a,t){return t?`<caption class="sr-only">${e(`${n("kitchen.title")}: ${n(a.labelKey)}`)}</caption>`:`
    <caption class="summary-matrix-caption">
      <time datetime="${e(a.dateId)}">${e(B(a.dateId))}</time>
    </caption>
  `}function S(a){return{breakfast:"☕",lunch:"🍝",dinner:"🍽"}[a]||"•"}function v(a){const t=String(a?.mealTypeId||"").trim().toLowerCase(),s=t?n(`meal.type.${t}`):"",r=String(a?.label||"").trim();return s&&s!==`meal.type.${t}`?s:r}function y(a,t,s,r){return`
    <tr class="${t}">
  ...
  `}function $(a){return`<span class="summary-matrix-empty" aria-hidden="true">—</span><span class="sr-only">${e(a)}</span>`}function p(a,t){return a.dayIndex>t.index?" summary-matrix-next-date":""}function x(a){return[n("summary.today"),n("summary.tomorrow"),n("summary.dayAfterTomorrow")][a]||n("summary.followingDay")}function g(a){const[t,s,r]=String(a).split("-").map(Number);return new Intl.DateTimeFormat(f(),{day:"2-digit",month:"2-digit",year:"numeric"}).format(new Date(t,s-1,r))}function B(a){const[t,s,r]=String(a).split("-").map(Number);return!t||!s||!r?"":new Intl.DateTimeFormat(f(),{weekday:"long",day:"numeric",month:"long"}).format(new Date(t,s-1,r))}function Y(a,t){const s=[...a.querySelectorAll(`[data-${t}-screen]`)];if(s.length===0)return 0;const r=s.reduce((m,i)=>Math.abs(i.getBoundingClientRect().left-a.getBoundingClientRect().left)<Math.abs(m.getBoundingClientRect().left-a.getBoundingClientRect().left)?i:m);return Number(r.dataset[`${t}Screen`])===1?1:0}function A(a,t,s){a.querySelectorAll(`[data-${t}-screen]`).forEach(r=>{r.setAttribute("aria-hidden",String(Number(r.dataset[`${t}Screen`])!==s))})}export{X as mountSummaryMatrix,q as scrollSummaryMatrix};
