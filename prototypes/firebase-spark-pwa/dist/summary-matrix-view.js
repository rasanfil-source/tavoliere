import{t as n,getLocale as b}from"./i18n/i18n.mjs?v=20260822a";import{escapeHtml as s}from"./html-utils.js?v=20260816g";import{formatDietLabel as W,normalizeDietCode as z}from"./diet-utils.mjs?v=20260818w";import{buildKitchenMatrixScreens as O,buildSummaryMatrixScreens as U}from"./summary-matrix-model.js?v=20260820i";let I=0;function ha(a,{days:t=[],operationDays:e=[],kitchen:r=!1,layout:o="classic",residentLabel:m="name",activeIndex:i=0,onActiveIndexChange:u=()=>{}}={}){const l=r?O(t,e):U(t,[],e);if(o==="future"&&!r){G(a,l,m,i,u);return}if(l.every(d=>d.columns.length===0)){a.innerHTML=`<p class="empty-state">${s(n("summary.noMeal"))}</p>`;return}const c=r?"kitchen":"summary",v=o==="international"?ia:V;a.innerHTML=`
  ...
  `}function na(a,t){if(t||a.mealTypeId!=="breakfast")return"";const e=a.breakfastPlanned===!0,r=n(e?"summary.breakfastPlanned":"summary.breakfastNotPlanned");return`<span class="summary-matrix-breakfast-status summary-matrix-breakfast-${e?"yes":"no"}"><span aria-hidden="true">${e?"✓":"×"}</span>${s(r)}</span>`}function L(a){if(a.specialDiets.participantCount===0)return f(n("summary.noDiet"));const t=[...a.specialDiets.items].sort((e,r)=>y(e.tag).localeCompare(y(r.tag),b(),{numeric:!0}));return w(t)}function q(a){return a.specialDiets.participantCount===0?f(n("summary.noDiet")):w(a.specialDiets.items," summary-matrix-kitchen-diets")}function w(a,t=""){const e=[...a].sort((r,o)=>y(r.tag).localeCompare(y(o.tag),b(),{numeric:!0}));return`<ul class="summary-matrix-diets${t}">${e.map(r=>{const o=y(r.tag),m=Math.max(0,Math.floor(Number(r.count)||0)),i=m>1?`${o} (${m})`:o;return`<li>${s(i)}</li>`}).join("")}</ul>`}function y(a){const t=z(a);return/^\d+$/.test(t)?t:W(t)}function E(a,{compactActions:t=!1,residentLabel:e="name"}={}){return a.names.length===0?f(n("summary.noName")):`<ul class="summary-matrix-names">${a.names.map(r=>{const o=r.dietTags.map($=>y($)),m=k(r.phone),i=String(r.displayName||"").trim(),u=i.split(/\s+/).filter(Boolean).slice(0,3).map($=>$[0]).join("").toUpperCase(),l=e==="signature"?r.signature||i:e==="initials"&&(r.initials||u||r.signature)||i,c=s(l),v=o.length?` <small>(${s(o.join(", "))})</small>`:"",h=`<span class="summary-matrix-person-name">${c}${v}</span>`,p=r.phoneConsent&&m?`<a class="summary-matrix-call" href="tel:${s(m)}" aria-label="${s(n("summary.callPerson",{name:r.displayName}))}"><span class="summary-matrix-phone-icon" aria-hidden="true">☎</span></a>`:"",d=r.whatsappEnabled&&r.phoneConsent&&m?`<a class="summary-matrix-whatsapp" href="https://wa.me/${s(m.replace(/\D/g,""))}" target="_blank" rel="noopener noreferrer" aria-label="${s(n("summary.messagePerson",{name:r.displayName}))}" title="WhatsApp"><img src="/icons/whatsapp.svg?v=20260808a" alt="" aria-hidden="true"></a>`:"";if(t&&(p||d)){const $=`summary-contact-popup-${++I}`;return`
