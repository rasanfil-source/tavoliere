import{t as n,getLocale as b}from"./i18n/i18n.mjs?v=20260818aa";import{escapeHtml as s}from"./html-utils.js?v=20260816g";import{formatDietLabel as H,normalizeDietCode as O}from"./diet-utils.mjs?v=20260818w";import{buildKitchenMatrixScreens as U,buildSummaryMatrixScreens as G}from"./summary-matrix-model.js?v=20260820i";let I=0;function $a(a,{days:t=[],operationDays:e=[],kitchen:r=!1,layout:m="classic",residentLabel:o="name",activeIndex:i=0,onActiveIndexChange:u=()=>{}}={}){const l=r?U(t,e):G(t,[],e);if(m==="future"&&!r){Y(a,l,o,i,u);return}if(l.every(d=>d.columns.length===0)){a.innerHTML=`<p class="empty-state">${s(n("summary.noMeal"))}</p>`;return}const c=r?"kitchen":"summary",S=m==="international"?na:X;a.innerHTML=`
    <div class="summary-matrix-track summary-layout-${m}${r?" summary-layout-kitchen":" summary-layout-diners"}" data-${c}-matrix-track aria-label="${s(n("summary.screensLabel"))}">
      ${l.map(d=>S(d,{kitchen:r,activeIndex:i,residentLabel:o})).join("")}
    </div>
  ...
  `;const o=a.querySelector("[data-summary-future-track]");let i=0;o?.addEventListener("scroll",()=>{window.clearTimeout(i),i=window.setTimeout(()=>{const u=Math.max(0,Math.min(1,Math.round(o.scrollLeft/Math.max(1,o.clientWidth+14))));o.querySelectorAll("[data-summary-future-screen]").forEach(l=>{l.setAttribute("aria-hidden",String(Number(l.dataset.summaryFutureScreen)!==u))}),m(u)},100)},{passive:!0}),window.requestAnimationFrame(()=>{o&&o.scrollTo({left:Math.max(0,r)*(o.clientWidth+14),behavior:"auto"})})}function _(a,t){const e=Array.isArray(a.names)?a.names:[],r=Number(a.specialDiets?.participantCount||0),m=Number(a.guestCount||0),o=Number(a.sickCount||0),i=Array.isArray(a.sickDiets)?a.sickDiets:[];return`
    <section class="summary-future-meal">
      <div class="summary-future-meal-main">
        <span class="summary-future-meal-icon" aria-hidden="true">${w(a.mealTypeId)}</span>
    ...
    <section class="summary-matrix-screen" data-${m}-screen="${a.index}" role="tabpanel" aria-hidden="${!o}">
      <table class="summary-matrix">
        ...
        <thead>
          ...
          <tr>
            ${a.columns.map(i=>`
              <th class="summary-matrix-meal-heading${h(i,a)}" scope="col"><span class="summary-matrix-meal-icon" aria-hidden="true">${w(i.mealTypeId)}</span><span class="summary-matrix-meal-label">${s(M(i))}</span>${ra(i,t)}</th>
            `).join("")}
          </tr>
        </thead>
        <tbody>
  ...
  `}function ra(a,t){if(t||a.mealTypeId!=="breakfast")return"";const e=a.breakfastPlanned===!0,r=n(e?"summary.breakfastPlanned":"summary.breakfastNotPlanned");return`<span class="summary-matrix-breakfast-status summary-matrix-breakfast-${e?"yes":"no"}"><span aria-hidden="true">${e?"✓":"×"}</span>${s(r)}</span>`}function L(a){if(a.specialDiets.participantCount===0)return f(n("summary.noDiet"));const t=[...a.specialDiets.items].sort((e,r)=>y(e.tag).localeCompare(y(r.tag),b(),{numeric:!0}));return v(t)}function P(a){return a.specialDiets.participantCount===0?f(n("summary.noDiet")):v(a.specialDiets.items," summary-matrix-kitchen-diets")}function v(a,t=""){const e=[...a].sort((r,m)=>y(r.tag).localeCompare(y(m.tag),b(),{numeric:!0}));return`<ul class="summary-matrix-diets${t}">${e.map(r=>{const m=y(r.tag),o=Math.max(0,Math.floor(Number(r.count)||0)),i=o>1?`${m} (${o})`:m;return`<li>${s(i)}</li>`}).join("")}</ul>`}function y(a){const t=O(a);return/^\d+$/.test(t)?t:H(t)}function q(a,{compactActions:t=!1,residentLabel:e="name"}={}){return a.names.length===0?f(n("summary.noName")):`<ul class="summary-matrix-names">${a.names.map(r=>{const m=r.dietTags.map($=>y($)),o=E(r.phone),i=String(r.displayName||"").trim(),u=i.split(/\s+/).filter(Boolean).slice(0,3).map($=>$[0]).join("").toUpperCase(),l=e==="signature"?r.signature||i:e==="initials"&&(r.initials||u||r.signature)||i,c=s(l),S=m.length?` <small>(${s(m.join(", "))})</small>`:"",g=`<span class="summary-matrix-person-name">${c}${S}</span>`,p=r.phoneConsent&&o?`<a class="summary-matrix-call" href="tel:${s(o)}" aria-label="${s(n("summary.callPerson",{name:r.displayName}))}"><span class="summary-matrix-phone-icon" aria-hidden="true">☎</span></a>`:"",d=r.whatsappEnabled&&r.phoneConsent&&o?`<a class="summary-matrix-whatsapp" href="https://wa.me/${s(o.replace(/\D/g,""))}" target="_blank" rel="noopener noreferrer" aria-label="${s(n("summary.messagePerson",{name:r.displayName}))}" title="WhatsApp"><img src="/icons/whatsapp.svg?v=20260808a" alt="" aria-hidden="true"></a>`:"";if(t&&(p||d)){const $=`summary-contact-popup-${++I}`;return`
    ...
    <section class="summary-matrix-screen summary-international-screen${ia(a)?" summary-screen-has-special":" summary-screen-ordinary"}" data-${m}-screen="${a.index}" role="tabpanel" aria-hidden="${!o}">
      ${t?`<h2 class="sr-only">${s(`${n("kitchen.title")}: ${n(a.labelKey)}`)}</h2>`:`<header class="summary-international-title"><time datetime="${s(a.dateId)}">${s(D(a.dateId))}</time></header>`}
