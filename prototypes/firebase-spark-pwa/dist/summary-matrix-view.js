import{t as r,getLocale as x}from"./i18n/i18n.mjs?v=20260818aa";import{escapeHtml as e}from"./html-utils.js?v=20260816g";import{formatDietLabel as q,normalizeDietCode as z}from"./diet-utils.mjs?v=20260818w";import{buildKitchenMatrixScreens as E,buildSummaryMatrixScreens as H}from"./summary-matrix-model.js?v=20260817h";let G=0;function oa(a,{days:t=[],operationDays:s=[],kitchen:n=!1,layout:m="classic",activeIndex:i=0,onActiveIndexChange:o=()=>{}}={}){const l=n?E(t,s):H(t,[],s);if(l.every(d=>d.columns.length===0)){a.innerHTML=`<p class="empty-state">${e(r("summary.noMeal"))}</p>`;return}const u=n?"kitchen":"summary",f=m==="international"?X:F;a.innerHTML=`
    ...
    <section class="summary-matrix-screen" data-${n}-screen="${a.index}" role="tabpanel" aria-hidden="${!m}">
      <table class="summary-matrix">
        ...
        <thead>
          ...
          <tr>
            ${a.columns.map(i=>`
              <th class="summary-matrix-meal-heading${$(i,a)}" scope="col"><span class="summary-matrix-meal-icon" aria-hidden="true">${w(i.mealTypeId)}</span><span class="summary-matrix-meal-label">${e(M(i))}</span>${Q(i,t)}</th>
            `).join("")}
          </tr>
        </thead>
        <tbody>
          ${a.hasGuestGroup?h(r("summary.guests"),"summary-matrix-row-guests",a,i=>String(i.guestCount)):""}
          ${h(r("summary.diningMeals"),"summary-matrix-row-meals",a,S)}
          ${a.hasSpecialDiets?h(r("summary.includedDiets"),"summary-matrix-row-diets",a,t?N:k):""}
          ${a.hasSickMeals?h(r("summary.sickMeals"),"summary-matrix-row-sick",a,C):""}
  ...
  `}function w(a){const t={breakfast:"☕",lunch:"🍝",dinner:"🍲"};return U({breakfast:"coffee",lunch:"sun",dinner:"moon"}[a],t[a]||"•")}function U(a,t="•"){const s=document.documentElement.dataset.interfaceStyle;if(!(s==="cool"||s==="urban")||!a)return t;const i={coffee:'<path d="M4 10h11v5a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4z"></path><path d="M15 11h2a3 3 0 0 1 0 6h-2"></path><path d="M6 5c0 1 .8 1.4.8 2.4S6 8.8 6 9.5M10 5c0 1 .8 1.4.8 2.4S10 8.8 10 9.5"></path>',sun:'<circle cx="12" cy="12" r="4"></circle><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"></path>',moon:'<path d="M20.5 14.2A8.5 8.5 0 0 1 9.8 3.5 8.5 8.5 0 1 0 20.5 14.2z"></path>'}[a];return i?`<svg class="meal-line-icon meal-line-icon-${a}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" focusable="false" aria-hidden="true">${i}</svg>`:t}function M(a){const t=String(a?.mealTypeId||"").trim().toLowerCase(),s=t?r(`meal.type.${t}`):"",n=String(a?.label||"").trim();return s&&s!==`meal.type.${t}`?s:n}function h(a,t,s,n){return`
    <tr class="${t}">
      <th class="summary-matrix-label" scope="row">${e(a)}</th>
      ${s.columns.map(m=>`<td class="${$(m,s).trim()}">${n(m)}</td>`).join("")}
    </tr>
  `}function S(a){const t=a.total===1?"summary.cover.one":"summary.cover.other";return`<span class="summary-matrix-total">${a.total}</span><span class="summary-matrix-unit">${e(r(t))}</span>`}function C(a){if(a.sickCount===0)return p(r("summary.sickMeals"));const t=a.sickCount===1?"summary.tray.one":"summary.tray.other";return`<span class="summary-matrix-diet-total">${a.sickCount}</span><span class="summary-matrix-unit">${e(r(t))}</span>`}function I(a){return a.sickDiets.length===0?p(r("summary.noDiet")):g(a.sickDiets)}function Y(a){if(a.massStatus==="UNKNOWN")return p(r("summary.notSet"));const t=a.massStatus==="YES";return`<span class="summary-matrix-mass-${t?"yes":"no"}">${e(r(t?"summary.yes":"summary.no"))}</span>`}function _(a){return`
  ...
  `}function D(a){return a==="YES"?" summary-mass-state-yes":a==="NO"?" summary-mass-state-no":" summary-mass-state-unknown"}function J(a){const t=j(a).map(s=>`<td class="summary-matrix-mass-band${$(s,a)}${D(s.massStatus)}" colspan="${s.span}">${L(s)}</td>`).join("");return`
    <tr class="summary-matrix-row-mass summary-matrix-row-mass-band">
      <th class="summary-matrix-label" scope="row">${e(r("summary.mass"))}</th>
      ${t}
    </tr>
  `}function Q(a,t){if(t||a.mealTypeId!=="breakfast")return"";const s=a.breakfastPlanned===!0,n=r(s?"summary.breakfastPlanned":"summary.breakfastNotPlanned");return`<span class="summary-matrix-breakfast-status summary-matrix-breakfast-${s?"yes":"no"}"><span aria-hidden="true">${s?"✓":"×"}</span>${e(n)}</span>`}function k(a){if(a.specialDiets.participantCount===0)return p(r("summary.noDiet"));const t=[...a.specialDiets.items].sort((s,n)=>y(s.tag).localeCompare(y(n.tag),x(),{numeric:!0}));return g(t)}function N(a){return a.specialDiets.participantCount===0?p(r("summary.noDiet")):g(a.specialDiets.items," summary-matrix-kitchen-diets")}function g(a,t=""){const s=[...a].sort((n,m)=>y(n.tag).localeCompare(y(m.tag),x(),{numeric:!0}));return`<ul class="summary-matrix-diets${t}">${s.map(n=>{const m=y(n.tag),i=Math.max(0,Math.floor(Number(n.count)||0)),o=i>1?`${m} (${i})`:m;return`<li>${e(o)}</li>`}).join("")}</ul>`}function y(a){const t=z(a);return/^\d+$/.test(t)?t:q(t)}function T(a,{compactActions:t=!1}={}){return a.names.length===0?p(r("summary.noName")):`<ul class="summary-matrix-names">${a.names.map(s=>{const n=s.dietTags.map(c=>y(c)),m=V(s.phone),i=e(s.displayName),o=n.length?` <small>(${e(n.join(", "))})</small>`:"",l=`<span class="summary-matrix-person-name">${i}${o}</span>`,u=s.phoneConsent&&m?`<a class="summary-matrix-call" href="tel:${e(m)}" aria-label="${e(r("summary.callPerson",{name:s.displayName}))}"><span class="summary-matrix-phone-icon" aria-hidden="true">☎</span></a>`:"",f=s.whatsappEnabled&&s.phoneConsent&&m?`<a class="summary-matrix-whatsapp" href="https://wa.me/${e(m.replace(/\D/g,""))}" target="_blank" rel="noopener noreferrer" aria-label="${e(r("summary.messagePerson",{name:s.displayName}))}" title="WhatsApp"><img src="/icons/whatsapp.svg?v=20260808a" alt="" aria-hidden="true"></a>`:"";if(t&&(u||f)){const c=`summary-contact-popup-${++G}`;return`
          ...
          </li>`}return`<li>${l}<span class="summary-matrix-contact-actions">${u}${f}</span></li>`}).join("")}</ul>`}function V(a){const t=String(a||"").trim();return/^[+\d][\d\s()./-]{5,}$/.test(t)?t:""}function X(a,{kitchen:t,activeIndex:s}){const n=t?"kitchen":"summary",m=a.index===s;return`
    <section class="summary-matrix-screen summary-international-screen${Z(a)?" summary-screen-has-special":" summary-screen-ordinary"}" data-${n}-screen="${a.index}" role="tabpanel" aria-hidden="${!m}">
  ...
  `}function Z(a){return a.columns.some(t=>t.guestCount>0||t.specialDiets.participantCount>0||t.sickCount>0||t.sickDiets.length>0)||a.notesByDate.length>0}function aa(a,{kitchen:t}){const s=t?N(a):k(a);return`
    <article class="summary-international-card summary-day-tone-${A(a.dayIndex)}${a.mealTypeId==="breakfast"?" summary-international-card-next":""}">
      ...
      <dl>
        ...
        <div><dt>${e(r("summary.diningMeals"))}</dt><dd>${S(a)}</dd></div>
        ${a.specialDiets.participantCount>0?`<div><dt>${e(r("summary.includedDiets"))}</dt><dd>${s}</dd></div>`:""}
        ${a.sickCount>0?`<div><dt>${e(r("summary.sickMeals"))}</dt><dd>${C(a)}</dd></div>`:""}
  ...
  `}function ta(a,t){const s=j(a);return s.length===0?"":`
    <section class="summary-international-mass${t?" summary-international-mass-kitchen":""}">
      <div class="summary-international-mass-segments">
        ${s.map((n,m)=>`
          <div class="summary-international-mass-group${m===0?" summary-international-mass-group-first":""}${$(n,a)}${D(n.massStatus)}" style="--mass-segment-span:${n.span}">
  ...
  `}function p(a){return`<span class="summary-matrix-empty" aria-hidden="true">—</span><span class="sr-only">${e(a)}</span>`}function sa(a,t){return a.dayIndex>t.index?" summary-matrix-next-date":""}function L(a){return`
