...
test('un amministratore già noto può riaccettare soltanto con un nuovo invito valido', () => {
  const invitationClaim = rules.match(/function invitationMembershipClaimIsValid\(centerId, adminUid\)[\s\S]*?\n    \}/)?.[0] || '';
  ...
  assert.match(adminRules, /resource\.data\.status == 'REVOKED'[\s\S]*resource\.data\.status == 'ACTIVE'[\s\S]*resource\.data\.role == 'ADMIN'[\s\S]*invitationMembershipClaimIsValid\(centerId, adminUid\)/);
...
test('solo il responsabile unico cambia titolo e seconda riga iniziali', () => {
  assert.match(rules, /'appDisplayName', 'appDisplaySubtitle', 'startupPresentationEnabled', 'updatedAt'/);
  ...
  assert.match(rules, /startupPresentationEnabled is bool/);
  assert.match(
    rules,
    /allow update: if settingsId == 'current'[\s\S]*affectedKeys\(\)[\s\S]*hasAny\(\['appDisplayName', 'appDisplaySubtitle', 'startupPresentationEnabled'\]\)[\s\S]*isCenterOwner\(centerId\)/
  );
});
